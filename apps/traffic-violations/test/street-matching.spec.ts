...
import {
	...
	type MatchInput,
	matchLocation,
} from "../src/lib/match-orchestrator.js";
import {
	levenshteinDistance,
...
import {
	normalizeLocation,
	normalizeStreetName,
} from "../src/lib/street-normalizer.js";
...
describe("Street Normalizer", () => {
	describe("normalizeStreetName", () => {
		it("should remove accents and uppercase", () => {
			...
			expect(normalizeStreetName("Rua Sítio dos Coqueiros")).toBe(
				"RUA SITIO DOS COQUEIROS",
			);
		});

		it("should expand common abbreviations", () => {
			expect(normalizeStreetName("R. DR. JOSE MARIA")).toBe(
				"RUA DOUTOR JOSE MARIA",
			);
			expect(normalizeStreetName("AV. PROF. JOSE")).toBe(
				"AVENIDA PROFESSOR JOSE",
			);
		});
	});
	...
	describe("normalizeLocation", () => {
		it("should extract clean street name from simple location", () => {
			const result = normalizeLocation(
				"AVENIDA BOA VIAGEM, EM FRENTE AO SEMAFORO N. 2784",
			);
		...
		it("should clean double prefix RUA RUA", () => {
			const result = normalizeLocation(
				"RUA RUA DOIS DE FEVEREIRO, AO LADO AO N. 529",
			);
		...
		it("should clean RUA AVENIDA corruption", () => {
			const result = normalizeLocation(
				"RUA AVENIDA ANTONIO DE GOES, EM FRENTE AO N. 200",
			);
			expect(result.streetType).toBe("AVENIDA");
		...
		it("should handle abbreviated street types", () => {
			const result = normalizeLocation(
				"R. DR. JOSE MARIA, LADO OPOSTO AO POSTE",
			);
			expect(result.streetType).toBe("RUA");
		...
		it("should handle AV abbreviation", () => {
			const result = normalizeLocation(
				"AV DR JOSE RUFINO, SOB AO SEMAFORO N 044",
			);
			expect(result.streetType).toBe("AVENIDA");
		...
		it("should extract direction", () => {
			const result = normalizeLocation(
				"RUA DOS PALMARES SEMAFORO NR 030 SENTIDO SUBURBIO",
			);
		...
		it("should handle locations without recognized type", () => {
			const result = normalizeLocation(
				"CORREGO DO BARTOLOMEU, EM FRENTE AO N. 509",
			);
		...
		it("should extract post number", () => {
			const result = normalizeLocation(
				"RUA IMPERADOR PEDRO SEGUNDO, EM FRENTE AO POSTE N. B017383",
			);
...
describe("Street Matcher", () => {
	describe("levenshteinDistance", () => {
		...
		it("should return correct distance for substitutions", () => {
			...
			expect(levenshteinDistance("AVENIDA BOA VIAGEM", "RUA BOA VIAGEM")).toBe(
				6,
			);
		});
	...
	describe("matchLevenshtein", () => {
		it("should match with high similarity", () => {
			const results = matchLevenshtein(
				"AVENIDA BOA VIAGEM",
				SAMPLE_STREETS,
				0.8,
			);
...
describe("mapConfidence", () => {
	...
	it("should scale levenshtein confidence", () => {
		...
		expect(mapConfidence(0.9, "levenshtein")).toBe(0.8);
		expect(mapConfidence(0.85, "levenshtein")).toBe(0.7);
		expect(mapConfidence(0.7, "levenshtein")).toBe(0.5);
	});

	it("should scale trigram confidence", () => {
		expect(mapConfidence(0.8, "trigram")).toBe(0.85);
		expect(mapConfidence(0.6, "trigram")).toBe(0.7);
		expect(mapConfidence(0.3, "trigram")).toBe(0.3);
	});
});
