...
async function loadLocationDict(): Promise<Map<number, string>> {
	const basePath = join(import.meta.dirname, "../../src/db");
	const raw = await readFile(join(basePath, "dict_locais_v2.json"), "utf-8");
...
async function phase2_matchOrphans(
	...
	console.log(`   🔍 ${rows.length} orphan locations to text-match`);
...
async function phase3_applyMatches(): Promise<{ updated: number }> {
	...
	const matches = await db
		...
		.where(sql`${locationStreetMatches.matched_street_code} IS NOT NULL`);
...
async function printStats() {
	...
	const byMethod = await db
		...
		.from(locationStreetMatches)
		.where(sql`${locationStreetMatches.match_method} IS NOT NULL`)
