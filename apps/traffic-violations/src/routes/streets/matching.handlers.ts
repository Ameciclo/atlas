import { eq, count, desc, sql, and, isNotNull, isNull } from "drizzle-orm";
...
import { matchLocation, batchMatchLocations, computeMatchStats } from "../../lib/match-orchestrator.js";
...
> = async (c) => {
  ...
  try {
    ...
    const output = matchLocation(input, { streets, autoAcceptThreshold: 0.85, validationThreshold: 0.50 });
    ...
    const candidates = output.result.candidates.map((c) => ({
      ...
      neighborhood_name: c.street.neighborhoodName || null,
...
> = async (c) => {
  ...
  try {
    ...
    const outputs = batchMatchLocations(inputs, { streets, autoAcceptThreshold: 0.85, validationThreshold: 0.50 });
    ...
    return c.json(
      {
        ...
        total_auto_accepted: (totalMatched?.count || 0) - (totalPending?.count || 0),
  ...
  try {
    ...
    await db
      ...
      .set({
        ...
        validated_at: new Date(),
        needs_validation: false,
        matched_street_code: body.corrected_street_code ?? existing.matched_street_code,
      })
      .where(eq(locationStreetMatches.id, id));

    return c.json(
      { success: true, message: "Match rejected" },
      200,
    );
  } catch (error) {
