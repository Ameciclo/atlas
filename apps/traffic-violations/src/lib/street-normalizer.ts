...
function cleanStreetPrefix(text: string): string {
  ...
  cleaned = cleaned.replace(/^RUA\s+(AVENIDA|PRACA|TRAVESSA|ESTRADA|RUA)\b/i, "$1");
...
function extractStreetParts(text: string): {
  ...
  if (typeMatch && typeMatch[0]) {
    ...
    const streetName = nameEnd >= 0 ? rest.slice(0, nameEnd).trim() : rest.trim();
  ...
  const namePart = firstComma >= 0 ? cleaned.slice(0, firstComma).trim() : cleaned;
  const nameEnd =
    namePart.search(
...
export function normalizeLocation(raw: string): NormalizedLocation {
  ...
  if (direction) {
    textWithoutDirection = expanded.slice(
      0,
      expanded.lastIndexOf(direction),
    ).trim();
  }
