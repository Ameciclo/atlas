#!/usr/bin/env python3
"""
ETL v3: Normalize 19 raw files -> data/traffic-tickets-compiled.tsv (5 columns).
Handles 2 formats (Datastore TSV 2007-2012+2025, semicolon CSV 2013-2024).

Output columns (TSV, tab-separated, UTF-8):
  violation_date  agent_id  violation_dict_id  location_id  cttu_code

violation_dict_id references traffic_tickets_catalog.id
location_id references traffic_tickets_locations.location_id
...
CATALOG_PATH = os.path.join(DATA_DIR, "catalog_categories.json")
DICT_LOCAIS = os.path.join(DATA_DIR, "location-descriptions.tsv")
OUT_TSV = os.path.join(DATA_DIR, "traffic-tickets-compiled.tsv")
...
def load_location_dict():
    """Load location-descriptions.tsv -> mapping raw_string to location_id."""
    mapping = {}
    with open(DICT_LOCAIS, "r", encoding="utf-8") as f:
        reader = csv.DictReader(f, delimiter="\t")
        for row in reader:
            raw = row.get("raw_description", "").strip()
            loc_id = row.get("location_id", "").strip()
            if raw and loc_id:
                mapping[raw] = int(loc_id)
    return mapping

def load_catalog_dict():
    """Load catalog_categories.json -> mapping description to catalog_id (1-indexed array position).
    Maps both canonical_description and all known_variants for each entry.
    First match wins (early entries in the JSON take priority).
    """
    with open(CATALOG_PATH, "r", encoding="utf-8") as f:
        catalog = json.load(f)
    lookup = {}
    for i, entry in enumerate(catalog):
        cat_id = i + 1  # matches traffic_violations_catalog.id (serial from 1)
        canonical = entry.get("canonical_description", "").strip()
        if canonical and canonical not in lookup:
            lookup[canonical] = cat_id
        for variant in entry.get("known_variants", []):
            v = variant.strip()
...
def main():
    print("=" * 60)
    print("ETL v3: Normalizing 19 files -> data/traffic-tickets-compiled.tsv")
    print()
    
    # Load references
    print("Loading reference data...")
    cat_dict = load_catalog_dict()
    loc_dict = load_location_dict()
    print(f"  Catalog dict:   {len(cat_dict):,} unique descriptions")
    ...
    for year in sorted(TSV_FILES.keys()):
        ...
        with open(fpath, "r", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                # Lookup catalog id for this description
                cat_id = cat_dict.get(parsed["description"])
                if cat_id is None:
                    skipped["unmatched_catalog"] += 1
                    continue
