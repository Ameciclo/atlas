 ...
 */
const API_NAME = "traffic-tickets";
...
async function generateOpenAPISpec() {
	try {
		...
		const openAPIDoc = app.getOpenAPIDocument({
			openapi: "3.1.0",
			info: {
				title: "Traffic Tickets API",
				version,
				description: "API for managing traffic tickets data",
				contact: {
					name: "Atlas Team",
			...
			servers: [
				{
					url: `http://localhost:${process.env.PORT || "3044"}`,
					description: "Local development server",
				},
				{
					url:
						process.env.API_BASE_URL ||
						"https://api.ameciclo.org/traffic-tickets",
			...
			tags: [
				{
					...
					description: "Operations related to traffic tickets",
				},
				{
					name: "System",
