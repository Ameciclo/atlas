...
const nearbyInputSchema = z.object({
	lat: z.string().transform(Number).pipe(z.number().min(-90).max(90)).openapi({ example: "-8.0476" }),
	lng: z.string().transform(Number).pipe(z.number().min(-180).max(180)).openapi({ example: "-34.8770" }),
	radius: z.string().transform(Number).pipe(z.number().min(100).max(5000)).default("1000").openapi({ example: "1000" }),
});
...
const nearbyResponseSchema = z.object({
	...
	emergency_calls: z.object({
		annual_history: z.array(z.object({
			year: z.number(),
			total_calls: z.number(),
		})),
		last_month_data: z.object({
			month: z.string(),
			total_calls: z.number(),
		}).nullable(),
