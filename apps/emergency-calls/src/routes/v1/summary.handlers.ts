...
export const summary: AppRouteHandler<SummaryRoute> = async (c) => {
	...
	const [validResult] = await db
		...
		.from(emergencyCalls)
		.where(
			and(yearCondition, sql`${emergencyCalls.outcome_category} IS NOT NULL`),
		);
	...
	const topCityYearlyData = topCityResult?.municipio
		? await db
				...
				.where(
					and(
						yearCondition,
						eq(emergencyCalls.municipality, topCityResult.municipio!),
					),
				)
				.groupBy(sql`EXTRACT(YEAR FROM ${emergencyCalls.date})`)
				.orderBy(sql`EXTRACT(YEAR FROM ${emergencyCalls.date})`)
	...
	const finalizationData = await db
		...
		.where(
			and(
				yearCondition,
				sql`${emergencyCalls.finalization_category} IS NOT NULL`,
			),
		)
	...
	return c.json({
		...
		cidadeMaisViolenta: {
			...
			totalInvalidas: topCityResult
				? topCityYearlyData.reduce(
						(sum, d) => sum + Number(d.totalInvalidas),
						0,
					)
				: 0,
		...
		porCategoria: Object.entries(
			categoriesData.reduce(
				(acc, item) => {
					const bucket = normalizeCategory(item.categoria);
		...
		periodo: {
			...
			fim:
				end_year ??
				(periodResult?.ultimoMes
					? Number.parseInt(periodResult.ultimoMes.split(".")[0])
					: start_year),
