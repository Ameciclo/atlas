...
function normalizeCategories(raw: Record<string, number>): Record<string, number> {
	const result: Record<string, number> = { nao_informado: 0 };
	for (const bucket of CATEGORY_BUCKETS) result[bucket] = 0;
...
function normalizeAge(raw: Record<string, number>): Record<string, number> {
	const result: Record<string, number> = { nao_informado: Number(raw.nao_informado) || 0 };
