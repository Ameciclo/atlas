...
export const citySummary: AppRouteHandler<CitySummaryRoute> = async (c) => {
	...
	const cityConditions = [
		...
		...buildYearConditions(start_year, end_year),
	];
	...
	const years = Object.keys(accidentsPerYear)
		.map(Number)
...
export const cityGeoJSON: AppRouteHandler<CityGeoJSONRoute> = async (c) => {
	...
	for (const row of categoryRows) {
		...
		categoryByStreet[loc][subtype] =
			(categoryByStreet[loc][subtype] || 0) + row.count;
	}
	...
	const features = topStreets.map((street, index) => {
		...
		return {
			...
			geometry: (pcrData?.geometry as {
				type: string;
				coordinates?: unknown;
			}) || { type: "LineString", coordinates: [] },
			properties: {
				accidents_count: street.count,
				accidents_by_category: normalizeCategories(
					categoryByStreet[streetLocation] || {},
				),
...
export const streetProfiles: AppRouteHandler<StreetProfilesRoute> = async (
	...
	const byAccidentType = normalizeCategories(
		typeData.reduce(
			(acc, item) => {
...
export const streetRecords: AppRouteHandler<StreetRecordsRoute> = async (c) => {
	...
	const formattedRecords = records.map((record) => ({
		...
		category: normalizeCategory(record.subtype),
