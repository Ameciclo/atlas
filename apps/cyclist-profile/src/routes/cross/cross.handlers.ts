...
const METRIC_MAP: Record<string, MetricDef> = {
	count: { sql: "count(*)" },
	collision_count: {
		sql: "count(*) filter (where data->>'collisions' = 'Sim')",
	},
	...
	distance_time_avg: {
		sql: "round(avg((data->>'distance_time')::numeric), 1)",
	},
	...
	try {
		...
		const addIn = (source: string, vals: unknown) => {
			if (Array.isArray(vals) && vals.length > 0) {
				conditions.push(
					`${source} in (${vals.map((v) => esc(String(v))).join(", ")})`,
				);
			}
		};
		...
		const addRange = (source: string, min: unknown, max: unknown) => {
			if (min !== undefined && min !== null)
				conditions.push(`${source} >= ${Number(min)}`);
			if (max !== undefined && max !== null)
				conditions.push(`${source} <= ${Number(max)}`);
		};
		...
		const addBool = (source: string, val: unknown) => {
			if (val === true) conditions.push(`${source} = 'Sim'`);
				...
				conditions.push(
					`(${source} is null or ${source} = '' or ${source} = 'Nao')`,
				);
		};
		...
		if (filters.collided !== undefined)
			addBool("data->>'collisions'", filters.collided);
		addRange(
			"(data->'days_usage'->>'total')::int",
			filters.days_total_min,
			filters.days_total_max,
		);
		addRange(
			"(data->'days_usage'->>'working')::int",
			filters.days_working_min,
			filters.days_working_max,
		);
		addIn("data->>'years_using'", filters.years_using);
		addRange(
			"(data->>'distance_time')::int",
			filters.distance_time_min,
			filters.distance_time_max,
		);
		...
		if (filters.combines_transport !== undefined)
			addBool("data->>'transport_combination'", filters.combines_transport);
		...
		// GROUP BY
		const groupParts = allDims
			.map((dim) => FIELD_MAP[dim]?.source)
			.filter(Boolean) as string[];
		const groupStr =
			groupParts.length > 0 ? `group by ${groupParts.join(", ")}` : "";
		const havingStr = options.min_n
			? `having count(*) >= ${options.min_n}`
			: "";
		...
		const totalN = rows.reduce(
			(sum, row) => sum + (Number(row.count) || Number(row._total) || 0),
			0,
		);
		...
		if (options.min_n && data.some((d) => d.suppressed)) {
			warnings.push(
				`Algumas celulas foram suprimidas por terem n < ${options.min_n}.`,
			);
		}
