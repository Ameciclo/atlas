...
export const query: AppRouteHandler<QueryRoute> = async (c) => {
	...
	const addIn = (f: string, vals: unknown) => {
		if (Array.isArray(vals) && vals.length > 0) {
			conditions.push(
				`${f} in (${vals.map((v) => esc(String(v))).join(", ")})`,
			);
		}
	};
	...
	const selectParts = [
		`width_bucket(${source}, 1, 100, ${bins}) as bin_idx`,
		"count(*) as count",
	];
	...
	const statsParts = [
		"count(*)",
		`round(avg(${source}), 1) as avg`,
		`min(${source}) as min`,
		`round(percentile_cont(0.25) within group (order by ${source})::numeric, 1) as p25`,
		`round(percentile_cont(0.5) within group (order by ${source})::numeric, 1) as median`,
		`round(percentile_cont(0.75) within group (order by ${source})::numeric, 1) as p75`,
		`max(${source}) as max`,
	];
	...
	const statsRows =
		(statsResult as { rows: Record<string, unknown>[] }).rows || [];
