import { and, count, desc, eq, isNotNull, like, ne, sql } from "drizzle-orm";
import { db } from "../../../db/index.js";
import { trafficDeaths } from "../../../db/schema.js";
import type { AppRouteHandler } from "../../../lib/types.js";
import CID10Dict from "./CID10CAT.json" with { type: "json" };
...
> = async (c) => {
	...
	const cityId =
		cityIdRaw && cityIdRaw > 999999 ? Math.floor(cityIdRaw / 10) : cityIdRaw;
	const locationType =
	...
	conditions.push(isNotNull(trafficDeaths.linhaa));
	conditions.push(ne(trafficDeaths.linhaa, ""));

	const codigoLimpo = sql<string>`split_part(ltrim(${trafficDeaths.linhaa}, '*'), '*', 1)`;
	...
	const causas = rows.map((r) => ({
		codigo: r.codigo,
		descricao:
			CID10Dict[r.codigo.substring(0, 3) as keyof typeof CID10Dict] ?? r.codigo,
		total: r.total,
		percentual:
			totalGeral > 0
