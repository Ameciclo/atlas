...
export const getFiltrosV1Handler: AppRouteHandler<typeof getFiltrosV1> = async (c) => {
	const query = c.req.valid("query");
	
	// Normalize parameters (support both English and Portuguese)
	const filters = {
		tipoLocal: query.tipoLocal || (query.locationType === "residence" ? "residencia" : "ocorrencia"),
		anoInicio: query.anoInicio || query.startYear,
		modoTransporte: query.modoTransporte || query.transportMode ? [query.modoTransporte || query.transportMode] : undefined,
	};
