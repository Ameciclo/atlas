...
import { classifyFaixaEtaria, idadeObitoAnos } from "../../../lib/idade.js";
...
export const getFiltrosV1Handler: AppRouteHandler<typeof getFiltrosV1> = async (
	...
	const [
		...
		db.select({ total: count() }).from(trafficDeaths).where(where),
		db
			.select({
	...
	const dados = dadosRows.map((r) => ({
		...
		municipio: {
			...
			nome:
				r.city_code !== null
					? RMR_6_DIGIT_NAMES[r.city_code] || `Cidade ${r.city_code}`
					: "Desconhecido",
		},
		sexo: {
			codigo: r.sexo ?? "9",
		...
		racacor: {
			codigo: r.racacor ?? "9",
			descricao:
				r.racacor !== null ? RACACOR_MAP[r.racacor] || "Ignorado" : "Ignorado",
		},
		...
		localOcorrenciaObito: {
			codigo: r.lococor ?? "9",
			descricao:
				r.lococor !== null ? LOCOCOR_MAP[r.lococor] || "Ignorado" : "Ignorado",
		},
		causabas: r.causabas ?? "",
		total: 1,
	}));
