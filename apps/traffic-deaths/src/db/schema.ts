...
export const traffic_deaths = pgTable(
	"traffic_deaths",
	{
	...
	(table) => ({
		...
		check_sexo: check(
			"check_sexo",
			sql`${table.sexo} IN ('0', '1', '2') OR ${table.sexo} IS NULL`,
		),
		check_tipobito: check(
			"check_tipobito",
			sql`${table.tipobito} IN ('1', '2') OR ${table.tipobito} IS NULL`,
		),
		check_acidtrab: check(
			"check_acidtrab",
			sql`${table.acidtrab} IN ('1', '2', '9') OR ${table.acidtrab} IS NULL`,
		),
	}),
);
