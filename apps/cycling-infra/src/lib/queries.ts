 ...
 * optionally filtered by city. Defaults to RMR cities only.
 */
export async function getExistingInfraKm(
	...
	const rmrList = RMR_CITIES.join(",");
	const cityFilter = cityId
		? `AND (prw.osm_properties->>'city_id')::int = ${cityId}`
		: `AND (prw.osm_properties->>'city_id')::int IN (${rmrList})`;
 ...
 * Uses EXISTS to avoid double-counting relations that span multiple cities.
 */
export async function getPdcPlannedKm(): Promise<{
	...
	const result = await db.execute(`
		...
		) AND cir.pdc_km IS NOT NULL AND cir.pdc_typology IS NOT NULL
	...
	for (const way of waysRows) {
		...
		if (inRmr) {
			if (way.has_cycleway && hasRelation) {
				pdc_feito_km += length;
				pdc_feito_by_type[typology] =
					(pdc_feito_by_type[typology] || 0) + length;
			} else if (way.has_cycleway && !hasRelation) {
				realizado_fora_pdc_km += length;
			} else if (!way.has_cycleway && hasRelation) {
				pdc_nao_realizado_km += length;
				pdc_nao_realizado_by_type[typology] =
					(pdc_nao_realizado_by_type[typology] || 0) + length;
			}
		} else if (!cityId) {
