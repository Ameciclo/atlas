...
def main():
    ...
    # Load existing non_pdc_ways.json and pdc_ways.json for dedup
    non_pdc_path = DB_DIR / "non_pdc_ways.json"
    pdc_path = DB_DIR / "pdc_ways.json"
    existing_ids = set()

    if os.path.exists(non_pdc_path):
        print(f"\nLoading existing non_pdc_ways.json...")
        with open(non_pdc_path) as f:
            existing_ways = json.load(f)
        existing_ids.update(w["osm_id"] for w in existing_ways)
        print(f"Non-PDC existing: {len(existing_ways)} ways")

    if os.path.exists(pdc_path):
        print(f"Loading pdc_ways.json...")
        with open(pdc_path) as f:
            pdc_ways = json.load(f)
        pdc_ids = {w["osm_id"] for w in pdc_ways}
        existing_ids.update(pdc_ids)
        print(f"PDC existing: {len(pdc_ways)} ways")
