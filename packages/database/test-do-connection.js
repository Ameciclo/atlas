...
const DATABASE_URL =
  'postgresql://atlas:AVNS_NK_FoQ5fDFAz6tu7hzs@ameciclo-postgres-db-do-user-18311227-0.i.db.ondigitalocean.com:25060/atlas?sslmode=require'
...
// Test 5: Pass file path directly as string (your approach)
async function testWithPathString () {
  console.log('Test 5: Pass file path directly as string (your approach)')
  try {
    const client = new Client({
      connectionString: DATABASE_URL.replace('?sslmode=require', ''),
      ssl: {
...
;(async () => {
  ...
  const test5 = await testWithPathString()
  ...
  console.log('Test 5 (Path string):', test5 ? '✅' : '❌')
  console.log('='.repeat(60))
})()
