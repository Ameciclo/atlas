...
import { closeDatabase, createConnectedDatabase, type DatabaseConfig } from "./connection.js";
		...
		for (let i = 0; i < geoData.features.length; i += batchSize) {
			...
			const stationsData = batch.map((feature) => {
				...
				if (typeof longitude !== 'number' || typeof latitude !== 'number' || 
					...
					console.warn(`⚠️  Invalid coordinates for station ${props.name || props["@id"]}: [${longitude}, ${latitude}]`);
					return null;
				}
				
				// Validate required properties
				if (!props.name || !props["@id"]) {
					console.warn(`⚠️  Missing required properties for station: name=${props.name}, id=${props["@id"]}`);
					return null;
				}
				
				// Validate capacity
				const capacity = props.capacity ? Number.parseInt(props.capacity, 10) : 0;
				if (Number.isNaN(capacity)) {
					console.warn(`⚠️  Invalid capacity for station ${props.name}: ${props.capacity}`);
					return null;
				}
				
				// Debug first few entries
				if (processed < 3) {
					console.log(`🔍 Debug station ${props.name}: coordinates=[${longitude}, ${latitude}]`);
				}
			...
			console.log(`✅ Processed ${processed}/${geoData.features.length} stations`);
		}
