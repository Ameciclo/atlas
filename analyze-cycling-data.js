...
function analyzeCSVStructure(filename, data) {
  ...
  headers.forEach((header, i) => {
    const values = lines.slice(1, 6).map(line => line.split(',')[i]).filter(v => v);
...
function analyzeGeoJSONStructure(filename, data) {
  ...
  Object.entries(propAnalysis)
    ...
    .forEach(([prop, info]) => {
      ...
      console.log(`  • ${prop}:`);
      console.log(`    - Cobertura: ${coverage}% (${info.count}/${data.features.length})`);
      console.log(`    - Valores únicos: ${uniqueValues}`);
      console.log(`    - Exemplos: ${examples}`);
      
      if (uniqueValues <= 20 && uniqueValues > 1) {
        console.log(`    - Todos os valores: ${Array.from(info.values).sort().join(', ')}`);
      }
    });
  ...
  geometryTypes.forEach(geomType => {
    ...
    console.log(`    - Exemplo coordenadas: ${JSON.stringify(example.geometry.coordinates).substring(0, 100)}...`);
  });
...
const relationsData = fs.readFileSync(path.join(dataDir, 'relations.csv'), 'utf8');
analyzeCSVStructure('relations.csv', relationsData);

// Analisar ways.geojson
const waysData = JSON.parse(fs.readFileSync(path.join(dataDir, 'ways.geojson'), 'utf8'));
analyzeGeoJSONStructure('ways.geojson', waysData);

// Analisar ciclomapa.geojson
const ciclomapaData = JSON.parse(fs.readFileSync(path.join(dataDir, 'ciclomapa-Recife, Pernambuco, Brasil.geojson'), 'utf8'));
