...
function extractOpenApiRoutes(filePath) {
  ...
  const exportPattern = /export\s+const\s+(\w+)\s*=\s*createRoute\s*\(\s*\{[^}]*path:\s*['"`]([^'"`]*?)['"`][^}]*method:\s*['"`](\w+)['"`]/g;
...
function generateMissingRouteCode(missingRoutes) {
  ...
  for (const [fileName, routes] of Object.entries(routesByFile)) {
    const routerName = fileName.includes('analytics') ? 'analyticsRouter' : `${fileName}Router`;
    const importName = fileName.includes('analytics') ? 'analyticsRoutes' : `${fileName}Routes`;
    
    console.log(`\nconst ${routerName} = createRouter()`);
    for (const route of routes) {
      console.log(`\t// biome-ignore lint/suspicious/noExplicitAny: Dummy handlers for spec generation only`);
...
function checkOpenApiCoverage() {
  ...
  for (const app of apps) {
    ...
    console.log(`📄 generate-openapi.ts: ${fs.existsSync(generatorPath) ? '✅' : '❌'}`);
    ...
    if (allDefinedRoutes.length > 0) {
      ...
      for (const route of allDefinedRoutes) {
        ...
        console.log(`   ${status} ${route.method} ${route.path} (${route.exportName})`);
      }
    }
    
    // Verificar rotas faltantes
    const missingRoutes = allDefinedRoutes.filter(route => 
      !registeredRoutes.includes(route.exportName)
    );
