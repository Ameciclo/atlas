...
const services = [
	...
	},
	{
		name: "shared-bike",
		port: 3015,
		endpoints: ["/health", "/v1/stations", "/v1/stations/1"],
	},
	{
		name: "cycling-infra",
		port: 3020,
...
async function testEndpoint(service, endpoint) {
	...
	try {
		...
		const timeout =
			endpoint.includes("/violations") || endpoint.includes("/streets/")
				? 10000
				: 2000;
...
async function testService(service) {
	...
	const status =
		successCount === service.endpoints.length
			? "✅"
			: successCount > 0
				? "⚠️"
				: "❌";
	console.log(
		`${status} ${service.name}: ${successCount}/${service.endpoints.length} endpoints working`,
	);
